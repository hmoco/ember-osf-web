...
function getCookieAuthUrl(nextUri) {
    const next = nextUri || config.OSF.redirectUri;
    const loginUri = `${config.OSF.url}login/?next=${encodeURIComponent(next)}`;
...
function getTokenFromHash(hashInput) {
    const hash = hashInput.substring(1).split('&');
